	...
	if ( $user = $userservice->db->sql_fetchrow($qid) ) {
		if ( "0" == $user['uStatus'] ) {
			...
			if ( $qid = $userservice->db->sql_query($sql) ) {
				$tplVars['msg'] = "Thank you.  Your account has been activated.  You may now <a href='$root/login/'>login</a>.";
			} else {
				$tplVars['error'] = "There was an error activating your account.  Please contact the site administrator.";
			}
		} else {
			$tplVars['error'] = "This account was already activated.  You may <a href='$root/login/'>login</a>.";
		}
	} else {
